...
(function() {
    ...
    window.AurumFoxEngine = {
        ...
        // Добавлено больше игнорируемых терминов для точности
        IGNORE_TERMS: ["individual claim", "use 'claim all'", "total unclaimed profit", "unclaimed profit", "yield farming active", "yield farming", "days", "tier", "select", "period", "tab", "switch", "dashboard", "menu", "nav", "amount", "input", "value", "field", "баланс", "go to", "open", "view", "earned rewards", "total staked"],
        ...
        init() {
            ...
            console.log("%c👑 OMNI-BRAIN v20.5: PRECISION TARGETING ENABLED", "color: #00ff88; font-weight: bold; background: black; padding: 8px 20px; border: 2px solid #00ff88; border-radius: 5px;");
        },
        ...
        deepDiscovery() {
            // ИСПРАВЛЕНО: Сначала выбираем кнопки, ссылки и мелкие элементы. DIV берем в последнюю очередь.
            ...
            els.forEach(el => {
                ...
                if (text.includes('yield farming active') || text.includes('total unclaimed profit') || text.includes('individual claim')) {
                    return;
                }

                // ИСПРАВЛЕНО: Игнорируем DIV, если он слишком большой (чтобы не кликало по пустому фону блока)
        ...
        bind(el, action) {
            ...
            el.addEventListener('click', async (e) => {
                ...
                // Проверка, что клик не на пустом месте большого блока
