...
// ============================================================
// 👑 AURUM FOX: OMEGA SMART ENGINE v11.0 - ROYAL EDITION
// ============================================================
...
window.AurumFoxEngine = {
    ...
    // "Богатые" фразы для успеха и ошибок
    ROYAL_PHRASES: {
        SUCCESS: ["RICH MODE ✅", "GOLDEN DONE 👑", "ASSETS SECURED 💎", "ROYAL TRANSACTION", "AURUM TOUCH ✨", "BULLISH ✅"],
        ERROR:   ["MARKET REJECT ❌", "NOT TODAY ⚡", "TRY AGAIN, BOSS", "LIQUIDITY WAIT", "FOX DENIED"]
    },

    INTELLIGENT_MAP: {
        "CLAIM":        { ids: ["collect-all-profit-btn", "claim-all-rewards-btn"], keywords: ["collect", "claim", "profit"], royal: "COLLECTED 💰" },
        "INIT_STAKE":   { ids: ["create-staking-account-btn"], keywords: ["create staking", "init stake"], royal: "CONTRACT OPENED" },
        "MAX_STAKE":    { ids: ["stake-max-btn"], keywords: ["max"], context: "stake", royal: "ALL IN 🚀" },
        "STAKE":        { ids: ["stake-afox-btn"], keywords: ["stake afox", "stake now"], royal: "STAKED 👑" },
        "MAX_UNSTAKE":  { ids: ["unstake-max-btn"], keywords: ["max"], context: "unstake", royal: "MAX OUT" },
        "UNSTAKE":      { ids: ["unstake-afox-btn"], keywords: ["unstake afox", "withdraw"], royal: "WITHDRAWN" },
        "REFUND":       { ids: ["close-account-refund-btn"], keywords: ["close account", "refund"], royal: "REFUNDED" },
        "COLLATERAL":   { ids: ["collateralize-btn"], keywords: ["collateralize", "enable collateral"], royal: "COLLATERAL ON" },
        "DECOLLATERAL": { ids: ["decollateralize-btn"], keywords: ["decollateralize", "remove collateral"], royal: "COLLATERAL OFF" },
        "BORROW":       { ids: ["execute-borrowing-btn"], keywords: ["execute borrow", "borrowing"], royal: "LOAN SECURED" },
        "REPAY":        { ids: ["repay-debt-btn"], keywords: ["repay debt"], royal: "DEBT PAID" },
        "REPAY_CLOSE":  { ids: ["repay-close-loan-btn"], keywords: ["repay & close", "close loan"], royal: "FREEDOM ✨" }
    },

    getRandom(type) {
        const list = this.ROYAL_PHRASES[type];
    ...
    notify(msg, type = "SYSTEM") {
        if (typeof window.showFoxToast === 'function') {
            ...
            console.log(`%c[${type}] ${msg}`, "color: #FFD700; font-weight: bold; text-shadow: 0 0 5px gold;");
        }
    },
    ...
    async getFreshBalance(mint) {
        try {
            ...
            if (!addr || !window.solanaWeb3) return 0n;
    ...
    init() {
        ...
        this.smartScan();
        setInterval(() => this.smartScan(), 2000);
        console.log("%c👑 AURUM FOX OMEGA v11.0: ROYAL MODE ON", "color: gold; font-size: 14px; font-weight: bold;");
    },

    smartScan() {
        const allButtons = document.querySelectorAll('button, a, .fox-btn, .btn');
        allButtons.forEach(btn => {
            if (btn.dataset.foxSynced === "true") return;
            for (const [action, config] of Object.entries(this.INTELLIGENT_MAP)) {
                const text = btn.innerText.toLowerCase();
                const id = btn.id;
                if (config.ids.includes(id) || config.keywords.some(kw => text.includes(kw))) {
                    btn.dataset.foxSynced = "true";
                    btn.dataset.foxAction = action;
                    btn.classList.add('fox-royal-btn');
                    btn.onclick = null; 
    ...
    async handleInteraction(el, action) {
        ...
        if (!userAddr) {
            this.notify("BOSS, CONNECT WALLET!", "ERROR");
            return;
        }

        const originalHTML = el.innerHTML;
        const config = this.INTELLIGENT_MAP[action];
        ...
        try {
            switch (action) {
                case "MAX_STAKE": case "MAX_UNSTAKE":
                    ...
                    break;
                case "INIT_STAKE": await this.ensureExecution(window.createStakingAccount, [0]); break;
                case "STAKE": await this.ensureExecution(window.stakeAfox); break;
                case "UNSTAKE": await this.ensureExecution(window.unstakeAfox); break;
                case "CLAIM": await this.ensureExecution(window.claimAllRewards, [[0]]); break;
                case "COLLATERAL": await this.ensureExecution(window.executeCollateral); break;
                case "DECOLLATERAL": await this.ensureExecution(window.executeDecollateral); break;
                case "BORROW": await this.ensureExecution(window.executeBorrow); break;
                case "REPAY": await this.ensureExecution(window.executeRepay); break;
                case "REPAY_CLOSE": await this.ensureExecution(window.executeRepay, ["max"]); break;
                case "REFUND": await this.ensureExecution(window.closeStakingAccount); break;
            }
            
            // "Козырный" ответ на кнопке
            el.innerHTML = `<span style="color: #00ff88; font-weight: bold;">${config.royal || this.getRandom('SUCCESS')}</span>`;
            this.notify(this.getRandom('SUCCESS'), "SUCCESS");

        } catch (err) {
            console.error(err);
            el.innerHTML = `<span style="color: #ff4444;">${this.getRandom('ERROR')}</span>`;
            this.notify("DENIED BY BLOCKCHAIN", "ERROR");
        } finally {
            setTimeout(() => {
                el.innerHTML = originalHTML;
                el.dataset.loading = "false";
            }, 2000); // 2 секунды пафоса перед возвратом текста
        }
    },
    ...
    async logicMax(type) {
        ...
        const input = document.getElementById(type === 'stake' ? 'stake-input-amount' : 'unstake-input-amount') || document.querySelector(`input[placeholder*="${type}"]`);
        if (input) {
    ...
    async ensureExecution(fn, args = []) {
        if (typeof fn !== 'function') {
            this.notify("SUMMONING CONTRACT...", "WAIT");
            await new Promise(r => setTimeout(r, 1200));
            if (typeof fn !== 'function') throw new Error("VOID_LOGIC");
        }
        return await fn(...args);
    },

    injectGlobalStyles() {
        if (document.getElementById('fox-royal-styles')) return;
        const style = document.createElement('style');
        style.id = 'fox-royal-styles';
        style.innerHTML = `
            .fox-royal-btn:hover { 
            ...
            .fox-royal-loader {
                ...
                animation: foxSpin 0.7s linear infinite;
                filter: drop-shadow(0 0 5px gold);
            }
            @keyframes foxSpin { to { transform: rotate(360deg); } }
            [data-loading="true"] { pointer-events: none !important; cursor: wait; }
        `;
        document.head.appendChild(style);
    }
};
