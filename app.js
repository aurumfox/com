...
(function() {
    ...
    window.AurumFoxEngine = {
        ...
        ROYAL_PHRASES: {
            SUCCESS: ["TRANSACTION SUCCESS", "SECURED", "DISPATCHED", "COMPLETED", "VERIFIED"],
            ERROR:   ["DECLINED", "VOID ASSETS", "REJECTED", "FAILED", "RETRYING"],
        },
        ...
        INTEL_MAP: {
            "CLAIM":        { terms: ["collect all profit", "claim all rewards"], royal: "REWARDS CLAIMED" },
            "INIT_STAKE":   { terms: ["create staking account"], royal: "ACCOUNT READY" },
            "MAX":          { terms: ["max"], royal: "MAXIMUM SET" },
            "STAKE":        { terms: ["stake afox"], royal: "ASSETS SECURED" },
            "UNSTAKE":      { terms: ["unstake afox"], royal: "ASSETS RELEASED" },
            "REFUND":       { terms: ["close account & refund sol"], royal: "SOL REFUNDED" },
            "COLLATERAL":   { terms: ["collateralize"], royal: "COLLATERAL ACTIVE" },
            "DECOLLATERAL": { terms: ["decollateralize"], royal: "COLLATERAL CLOSED" },
            "BORROW":       { terms: ["execute borrowing"], royal: "BORROWED" },
            "REPAY":        { terms: ["repay debt"], royal: "DEBT SETTLED" },
            "REPAY_CLOSE":  { terms: ["repay & close loan"], royal: "LOAN TERMINATED" }
        },
        ...
        init() {
            ...
            console.log("%c👑 OMNI-BRAIN v20.5: PREMIUM INTERFACE ACTIVE", "color: #00ff88; font-weight: bold; background: black; padding: 8px 20px; border: 2px solid #00ff88; border-radius: 5px;");
        },
        ...
        async handle(el, action) {
            ...
            try {
                ...
                el.innerHTML = `<span style="color: #00ff88; font-weight: bold; letter-spacing: 0.5px;">${royalTxt}</span>`;
                this.safeNotify(`${action} VERIFIED`, "SUCCESS");
            } catch (err) {
        ...
        async smartLogicMax(btn) {
            ...
            if (input) {
                ...
                this.safeNotify(`SET MAXIMUM: ${balance}`, "SUCCESS");
            } else {
        ...
        injectGlobalStyles() {
            ...
            style.innerHTML = `
                [data-loading="true"] { pointer-events: none !important; opacity: 0.9; }
                .fox-loader {
