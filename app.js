...
(function() {
    ...
    window.AurumFoxEngine = {
        ...
        // ЖЕСТКИЙ ИГНОР: Эти фразы скрипт пропускает и не превращает в кнопки
        IGNORE_TERMS: [
            "yield farming active", "farming active", "individual", "notice", "zero fee", 
            "audited", "disclaimer", "advice", "fees", "staked", "rewards", "farming", 
            "unclaimed", "your staking", "utilize", "institutional", "liquidity", "health", "threshold"
        ],
        ...
        deepDiscovery() {
            ...
            els.forEach(el => {
                ...
                // 1. ПРИНУДИТЕЛЬНЫЙ ПРОПУСК ДЛЯ ЖЕЛТОЙ КНОПКИ (Yield Farming Active)
                const shouldIgnore = this.IGNORE_TERMS.some(term => text === term || text.includes(term));
                if (shouldIgnore) {
                    el.dataset.foxSynced = "true"; // Помечаем, чтобы больше не проверять, но не биндим клик
                    return;
                }

                // 2. ПРОВЕРКА ПО БЕЛОМУ СПИСКУ: только если фраза разрешена
                const isApproved = this.WHITE_LIST.some(item => {
                    const cleanItem = item.toLowerCase().trim();
                    return text === cleanItem || (text.includes(cleanItem) && text.length < cleanItem.length + 10);
                });

                if (!isApproved) return;

                // 3. ПРИВЯЗКА ДЕЙСТВИЯ (Только для разрешенных кнопок)
        ...
        bind(el, action) {
            ...
            el.addEventListener('click', async (e) => {
                ...
                // Не останавливаем событие, если это не наше действие
                if (!el.dataset.foxAction) return;
        ...
        injectGlobalStyles() {
            ...
            style.innerHTML = `
                ...
                @keyframes f-spin { to { transform: rotate(360deg); } }
                body * { cursor: default; }
                [data-fox-synced="true"][data-fox-action], button, a, input, [role="button"] { cursor: pointer !important; }
