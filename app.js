...
// ============================================================
// 👑 AURUM FOX: OMNI-BRAIN v20.1 - ULTIMATE MAXIMA (FIXED TIER)
// ============================================================
// ПРИМЕЧАНИЕ: Самая большая версия. Без сокращений.
// ИСПРАВЛЕНО: Теперь выбор Тира/Дней открывается корректно.
// Логика: Умный поиск игнорирует элементы выбора периода.
// ============================================================
...
(function() {
    ...
    window.AurumFoxEngine = {
        ...
        // Ключевые слова для исключения (чтобы меню выбора дней работало)
        IGNORE_TERMS: ["days", "tier", "select", "period", "дней", "выбрать", "тир"],
        ...
        init() {
            ...
            console.log("%c👑 OMNI-BRAIN v20.1: TIER FIX ACTIVE", "color: gold; font-weight: bold; background: black; padding: 8px 20px; border: 2px solid gold; border-radius: 5px;");
        },
        ...
        deepDiscovery() {
            ...
            els.forEach(el => {
                ...
                // ПРОВЕРКА НА ИСКЛЮЧЕНИЯ (ТИР/ДНИ)
                if (this.IGNORE_TERMS.some(term => senseData.includes(term))) {
                    return; // Пропускаем, чтобы меню открылось
                }
        ...
        bind(el, action) {
            ...
            el.addEventListener('click', async (e) => {
                // Если это не системное исключение, блокируем стандарт и крутим анимацию
                e.preventDefault(); 
                e.stopPropagation();
