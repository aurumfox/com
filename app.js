...
const Views = {
    renderInitStake() {
        ...
        wrapper.id = 'initStakeView';
        wrapper.className = 'max-w-7xl mx-auto';
        
        // Вставляем всю «начинку» (тут весь твой HTML)
        wrapper.innerHTML = `
            <div class="flex justify-between items-center mb-8">
                <h1 class="text-3xl font-bold">Initialize Staking: Security Audit</h1>
                <button id="backBtn" class="text-gray-400 hover:text-white px-4 py-2 border border-white/10 rounded-xl transition-all">← Back to Staking</button>
            </div>
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <button id="confirmInitBtn" class="w-full mt-8 py-4 rounded-xl bg-emerald-500/20 border border-emerald-500/50 hover:bg-emerald-500/30 font-bold text-white transition-all shadow-lg shadow-emerald-500/10 cursor-pointer">
        ...
        // Навешиваем события уже после того, как элементы появились в DOM
        document.getElementById('backBtn').addEventListener('click', () => wrapper.remove());
        document.getElementById('confirmInitBtn').addEventListener('click', () => {
            console.log("Транзакция инициализации запущена...");
        });
    }
};
...
document.addEventListener('click', (e) => {
    if (e.target.matches('[data-target="initStakeView"]')) {
        Views.renderInitStake();
    }
});
