...
/**
 * AURUM FOX: ULTRA-CONNECT ENGINE (HYBRID PC/MOBILE)
 * Система принудительного возврата и умного дисконнекта
 */
...
const syncWalletUI = (isConnected, address = null) => {
    ...
    if (isConnected && address) {
        ...
        btn.style.background = "linear-gradient(135deg, #1a1a1a 0%, #000 100%)";
        btn.style.border = "1px solid #00ff7f";
        btn.style.color = "#00ff7f";
    } else {
        btn.innerHTML = `🦊 Connect Wallet`;
        btn.style.background = ""; // Возврат к CSS стилям темы
        btn.style.border = "";
        btn.style.color = "";
    }
};
...
async function toggleWalletAction() {
    ...
    // Метка времени для контроля зависания
    ...
    try {
        if (!AurumFoxEngine.isWalletConnected) {
            ...
            if (!provider) {
                if (isMobile) {
                    // Глубокая ссылка для открытия сайта сразу внутри Phantom Browser
                    const dappUrl = window.location.href.split('://')[1];
                    window.location.href = `https://phantom.app/ul/browse/${dappUrl}`;
                } else {
                    AurumFoxEngine.notify("Install Phantom Wallet", "ERROR");
            ...
            AurumFoxEngine.notify("Wallet Verified", "SUCCESS");

            // ПРИНУДИТЕЛЬНЫЙ ВЫХОД ИЗ КОШЕЛЬКА (для мобилок)
            if (isMobile) {
                // Если прошло больше 1.5 сек, значит юзера "зажевало" в приложении
                setTimeout(() => {
                    AurumFoxEngine.notify("Returning to terminal...", "AUTO_BACK");
                    window.location.replace(window.location.pathname + window.location.search);
                }, 1200);
            }

        } else {
            // ЛОГИКА ДИСКОННЕКТА (Чистый выход)
            ...
            syncWalletUI(false);
            AurumFoxEngine.notify("Session Ended", "WALLET_DISCONNECTED");

            // Принудительная очистка кэша сессии для мобилок при выходе
            if (isMobile) {
                setTimeout(() => {
        ...
        AurumFoxEngine.notify(err.code === 4001 ? "User Rejected" : "System Error", "FAILED");
        syncWalletUI(false);
    } finally {
...
/**
 * УМНЫЙ МОНИТОРИНГ ФОКУСА
 * Если юзер вернулся на вкладку вручную - подтягиваем статус мгновенно
 */
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
        const provider = window.solana || window.phantom?.solana;
        if (provider?.isConnected && !AurumFoxEngine.isWalletConnected) {
