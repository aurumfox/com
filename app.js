...
// ============================================================
// 👑 AURUM FOX: OMNI-BRAIN v20.3 - ULTIMATE MAXIMA (SHIELD+)
// ============================================================
// ПРИМЕЧАНИЕ: Самая большая версия. Без сокращений.
// ИСПРАВЛЕНО: Полная свобода для полей ввода (div/span inputs).
// Вкладки и навигация теперь работают идеально.
// ============================================================
...
(function() {
    ...
    window.AurumFoxEngine = {
        ...
        // Слова-исключения для вкладок и полей ввода
        IGNORE_TERMS: ["days", "tier", "select", "period", "дней", "выбрать", "тир", "tab", "switch", "dashboard", "menu", "nav", "amount", "input", "value", "field", "баланс"],
        ...
        init() {
            ...
            console.log("%c👑 OMNI-BRAIN v20.3: INPUT PROTECTION ACTIVATED", "color: #00ff88; font-weight: bold; background: black; padding: 8px 20px; border: 2px solid #00ff88; border-radius: 5px;");
        },
        ...
        deepDiscovery() {
            ...
            els.forEach(el => {
                // ЖЕСТКАЯ ПРОВЕРКА: Если это реальный инпут или область для печати - ИГНОР
                if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA' || el.isContentEditable) return;
                
                // ПРОВЕРКА: Если внутри этого DIV есть инпут - не трогаем сам DIV
                ...
                // ИСКЛЮЧЕНИЯ ПО КЛЮЧЕВЫМ СЛОВАМ (Вкладки, Поля, Тиры)
        ...
        bind(el, action) {
            ...
            el.addEventListener('click', async (e) => {
                // Если кликнули по чему-то, что похоже на поле ввода внутри кнопки (бывает и такое) - пропускаем
        ...
        async smartLogicMax(btn) {
            ...
            if (input) {
                ...
                this.safeNotify(`BALANCE SYNC: ${balance}`, "SUCCESS");
            }
        },
        ...
        injectGlobalStyles() {
            ...
            style.innerHTML = `
                ...
                @keyframes f-spin { to { transform: rotate(360deg); } }
                /* Насильно разрешаем клики и ввод для всего, что похоже на поля */
                input, textarea, [contenteditable="true"], .input-field, [class*="amount"] { 
