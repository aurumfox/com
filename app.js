...
// ============================================================
// 👑 AURUM FOX: OMNI-SCANNER v13.0 - ULTRA AUTONOMOUS
// ============================================================
...
window.AurumFoxEngine = {
    ...
    ROYAL_PHRASES: {
        SUCCESS: ["SUCCESS 👑", "SECURED 💎", "DISPATCHED ✨", "DONE, SIR"],
        ERROR:   ["DECLINED ❌", "VOID ASSETS", "REJECTED", "FAIL"],
    },

    // Расширенные смысловые паттерны
    INTEL_MAP: {
        "CLAIM":        { terms: ["collect", "claim", "profit", "harvest", "rewards"], royal: "COLLECTED 💰" },
        "INIT_STAKE":   { terms: ["create staking", "init stake", "setup staking"], royal: "INITIALIZED" },
        "MAX_STAKE":    { terms: ["max", "100%"], context: "stake", royal: "MAXED 🚀" },
        "STAKE":        { terms: ["stake afox", "stake now", "deposit"], royal: "STAKED 👑" },
    ...
    safeNotify(msg, type = "SYSTEM") {
        ...
        console.log(`%c[${type}] ${msg}`, "color: gold; font-weight: bold;");
    },

    init() {
        this.injectGlobalStyles();
        this.deepDiscovery();
        setInterval(() => this.deepDiscovery(), 1500); // Постоянный поиск новых элементов
        console.log("%c👑 OMNI-SCANNER v13.0: SYSTEM READY", "color: gold; font-weight: bold; background: black; padding: 3px 10px; border-radius: 5px;");
    },

    // Глубокий поиск: анализирует Текст, ID, Классы и даже Title
    deepDiscovery() {
        const els = document.querySelectorAll('button, a, [role="button"], .btn, .clickable');
    ...
    async handle(el, action) {
        ...
        try {
            // 1. Поиск функции в window или вложенных объектах (авто-нащупывание)
            ...
            switch (action) {
                case "MAX_STAKE": case "MAX_UNSTAKE":
                    ...
                    break;
                case "INIT_STAKE": await this.execute(fn || window.createStakingAccount, [0]); break;
                case "STAKE":      await this.execute(fn || window.stakeAfox); break;
                case "UNSTAKE":    await this.execute(fn || window.unstakeAfox); break;
                case "CLAIM":      await this.execute(fn || window.claimAllRewards, [[0]]); break;
                case "REPAY_CLOSE": await this.execute(fn || window.executeRepay, ["max"]); break;
                default:           await this.execute(fn); break;
            }
            ...
            el.innerHTML = `<span style="color: gold;">${failTxt}</span>`;
            this.safeNotify("ACTION REJECTED", "ERROR");
        } finally {
            setTimeout(() => {
    ...
    // "Нащупывание" функции в JavaScript, если её имя отличается
    findContractFunction(action) {
        const map = {
            "STAKE": ["stakeAfox", "deposit", "stake"],
        ...
        for (let name of candidates) {
            ...
            if (window.app && typeof window.app[name] === 'function') return window.app[name];
        }
        return null;
    },
    ...
    async smartLogicMax(btn, type) {
        // Ищем ближайший инпут именно в этом блоке
