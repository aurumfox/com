...
document.addEventListener('click', function(event) {
    // 1. Ищем ближайший элемент с атрибутом data-view (наша кнопка)
...
function switchView(targetId) {
    // Получаем все блоки, которые у тебя скрываются через 'hidden'
    const allViews = [
    ...
    allViews.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            // Если ID совпадает — показываем, иначе скрываем
            if (id === targetId) {
