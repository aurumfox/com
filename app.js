...
(function() {
    ...
    window.AurumFoxEngine = {
        ...
        // ЖЕСТКИЙ ИГНОР: Эти фразы НИКОГДА не станут кнопками
        IGNORE_TERMS: [
            "yield farming active", "farming active", "active", "individual", "notice", "zero fee", 
        ...
        deepDiscovery() {
            ...
            els.forEach(el => {
                ...
                // 1. ПРОВЕРКА НА ИГНОР (Чтобы Yield Farming Active не стал кнопкой сбора)
                if (this.IGNORE_TERMS.some(term => text === term || text.includes(term))) {
                    el.dataset.foxSynced = "true"; // Помечаем как обработанное, чтобы игнорировать навсегда
                    return;
                }

                // 2. ПРОВЕРКА ПО БЕЛОМУ СПИСКУ: только если фраза в списке разрешенных
                ...
                // Если прошло все проверки, привязываем действие
        ...
        injectGlobalStyles() {
            ...
            style.innerHTML = `
                ...
                @keyframes f-spin { to { transform: rotate(360deg); } }
                body * { cursor: default; }
                [data-fox-synced="true"], button, a, input, [role="button"] { cursor: pointer !important; }
