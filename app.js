...
/**
 * 🦊 FOX WALLET CORE - СВЕРХМОЩНЫЙ КОННЕКТОР
 * Управляет только входом/выходом. Независим от остального кода.
 */
const FoxWalletCore = {
    isLocked: false,
    ...
    async connect() {
        ...
        try {
            ...
            // Принудительная запись во все глобальные переменные
            window.appState.walletPublicKey = resp.publicKey;
            window.appState.provider = provider;
            AurumFoxEngine.isWalletConnected = true;

            // Обновляем визуал через основной движок
            AurumFoxEngine.handleRealWalletSync(provider);
            AurumFoxEngine.notify("Wallet Royal Sync: Active", "SUCCESS");
    ...
    async disconnect() {
        ...
        try {
            ...
            this.isLocked = false;
            // Перезагрузка страницы через 1 сек для полной очистки памяти (опционально)
            // location.reload(); 
        }
    },
                ...
                case "STAKING_DEPOSIT":
                    ...
                    if (typeof window.stakeAfox === 'function') {
                        await window.stakeAfox(
                            program, appState.userStakingData.poolIndex, amountBN, 
                            window.AFOX_POOL_STATE_PUBKEY, appState.userStakingPDA, 
                            appState.userSourceAta, window.AFOX_POOL_VAULT_PUBKEY
                        );
                    }
                    break;
