...
(function() {
    ...
    window.AurumFoxEngine = {
        ...
        // ЖЕСТКИЙ ИГНОР: Исключаем только конкретную надпись, чтобы не мешать кнопке Collateralize
        IGNORE_TERMS: [
        ...
        deepDiscovery() {
            ...
            els.forEach(el => {
                ...
                // 1. ПРОВЕРКА НА ИГНОР (Исключаем только Yield Farming Active)
                if (this.IGNORE_TERMS.some(term => text === term)) {
