...
// Define Program ID for AFOX, if not already defined
// Ensure AFOX_TOKEN_MINT_ADDRESS and STAKING_PROGRAM_ID are already available from previous steps
const AFOX_TOKEN_MINT_ADDRESS =GLkewtq8s2Yr24o5LT5mzzEeccKuSsy8H5RCHaE9uRAd SolanaWeb3.PublicKey('GLkewtq8s2Yr24o5LT5mzzEeccKuSsy8H5RCHaE9uRAd'); // Replace with your actual AFOX mint address
// const STAKING_PROGRAM_ID = new SolanaWeb3.PublicKey('Your_STAKING_PROGRAM_ID'); // If used for staking

// Jupiter Aggregator API URL - for fetching quotes
...
const TOKEN_MINT_ADDRESSES = {
    ...
    // You can add other tokens, for example:
    // 'USDC': new SolanaWeb3.PublicKey('EPjFWdd5AufqSSqeM2qN1xzybapT8G4AV6Z6P5YDgJLK'),
    // 'USDT': new SolanaWeb3.PublicKey('Es9viGfLCrmrSrvQZ3c3Fxz4o8s2c1F18zQ1Y32G5eE'),
};

// --- UI ELEMENTS FOR SWAP SECTION ---
...
const maxAmountBtns = document.querySelectorAll('.max-amount-btn'); // Added selector for all MAX buttons

let currentWallet = null; // Will store the currently connected wallet
let currentQuote = null; // Will store the last quote received from Jupiter

// --- HELPER FUNCTIONS ---

// Function to format large numbers (from your existing scripts)
...
// Function to connect wallet (from your existing scripts)
async function connectWallet(displayElement, connectButton, walletDisplaySpan) {
    if (window.solana && window.solana.isPhantom) {
        try {
            await window.solana.connect();
            const publicKey = window.solana.publicKey.toString();
            displayElement.textContent = `Connected: ${publicKey.substring(0, 4)}...${publicKey.substring(publicKey.length - 4)}`;
            connectButton.style.display = 'none'; // Hide Connect button after connection
            walletDisplaySpan.style.display = 'block'; // Show address
            console.log('Wallet connected:', publicKey);
            currentWallet = window.solana; // Store wallet object
            // Call functions to update balances after connection
            updateSwapBalances();
            return publicKey;
        } catch (error) {
            console.error('Failed to connect wallet:', error);
            displayElement.textContent = 'Connection Failed';
            showNotification('Wallet connection failed!', 'error');
            return null;
        }
    } else {
        showNotification('Phantom wallet not found. Please install Phantom.', 'warning');
...
function showNotification(message, type = 'info', duration = 3000) {
    ...
    notification.className = `notification ${type}`; // 'info', 'success', 'warning', 'error'
...
// Update balances for token selection
async function updateSwapBalances() {
    ...
    const connection = new SolanaWeb3.Connection(SolanaWeb3.clusterApiUrl('devnet')); // Use 'mainnet-beta' for production

    const fromTokenMint = TOKEN_MINT_ADDRESSES[swapFromTokenSelect.value];
    // const toTokenMint = TOKEN_MINT_ADDRESSES[swapToTokenSelect.value]; // Not directly used here, but kept for context

    // Balance for "From" token
    if (fromTokenMint.toBase58() === TOKEN_MINT_ADDRESSES['SOL'].toBase58()) {
        const solBalance = await connection.getBalance(currentWallet.publicKey);
        ...
        try {
            ...
            if (tokenAccount.value.length > 0) {
                ...
                swapFromBalanceSpan.textContent = `${formatBigInt(amount, decimals)} ${swapFromTokenSelect.value}`;
            } else {
    ...
    // You can also update the balance for the "To" token here if you need to display it
}

// Function to fetch a quote from Jupiter Aggregator
async function getQuote() {
    ...
    const fromMint = TOKEN_MINT_ADDRESSES[swapFromTokenSelect.value].toBase58();
    const toMint = TOKEN_MINT_ADDRESSES[swapToTokenSelect.value].toBase58();
    const amount = parseFloat(swapFromAmountInput.value);

    if (isNaN(amount) || amount <= 0) {
        showNotification('Please enter a valid amount to swap.', 'warning');
        return;
    }

    let inputAmountLamports;
    // Determine the number of decimal places for the input token
    // For SOL, it's 9 (LAMPORTS_PER_SOL)
    // For other SPL tokens, you need to get it from their metadata or know it beforehand (like for AFOX)
    const decimalsFrom = (fromMint === TOKEN_MINT_ADDRESSES['SOL'].toBase58()) ? 9 : 6; // Assuming AFOX has 6 decimal places
    inputAmountLamports = new BN(amount * (10 ** decimalsFrom)).toString(); // Use BN for large numbers

    showNotification('Fetching best swap quote...', 'info');
    getQuoteBtn.disabled = true;
    executeSwapBtn.style.display = 'none';

    try {
        const response = await fetch(`${JUPITER_API_URL}/quote?inputMint=${fromMint}&outputMint=${toMint}&amount=${inputAmountLamports}&slippageBps=50`); // slippageBps=50 is 0.5% slippage
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(`Failed to fetch quote: ${errorData.error || response.statusText}`);
        }
        const data = await response.json();
        currentQuote = data; // Save the quote for swap execution

        const outputAmount = parseFloat(currentQuote.outAmount);
        const outputDecimals = (toMint === TOKEN_MINT_ADDRESSES['SOL'].toBase58()) ? 9 : 6; // Decimal places for the target token
        const formattedOutput = formatBigInt(outputAmount, outputDecimals); // Format the output amount

        swapToAmountInput.value = formattedOutput;
        priceImpactSpan.textContent = `${(currentQuote.priceImpactPct * 100).toFixed(2)}%`; // Percentage format
        lpFeeSpan.textContent = `${formatBigInt(currentQuote.lpFee.amount, outputDecimals)} ${swapToTokenSelect.value}`; // LP fee in tokens
        minReceivedSpan.textContent = `${formatBigInt(currentQuote.otherAmountThreshold, outputDecimals)} ${swapToTokenSelect.value}`; // Minimum amount to receive

        showNotification('Quote received successfully!', 'success');
        executeSwapBtn.style.display = 'block'; // Show the execute button
    } catch (error) {
        console.error('Error fetching quote:', error);
        showNotification(`Error getting quote: ${error.message}`, 'error');
        currentQuote = null;
...
// Function to execute the swap via Jupiter Aggregator
async function executeSwap() {
    ...
    try {
        // Step 1: Request serialized transaction from Jupiter API
        const response = await fetch(`${JUPITER_API_URL}/swap`, {
            ...
            body: JSON.stringify({
                ...
                userPublicKey: currentWallet.publicKey.toBase58(),
                wrapUnwrapSOL: true, // Allows Jupiter to automatically wrap/unwrap SOL
            }),
        });
        ...
        // Attention: Phantom Wallet requires you to call signAndSendTransaction
        // to handle all necessary signings (including transactions created by Jupiter)
        const signedTransaction = await currentWallet.signAndSendTransaction(transaction); // Phantom will automatically sign and send

        showNotification('Transaction sent! Waiting for confirmation...', 'info');
        console.log('Swap transaction sent:', signedTransaction.signature);

        // Wait for transaction confirmation
        const connection = new SolanaWeb3.Connection(SolanaWeb3.clusterApiUrl('devnet')); // Use 'mainnet-beta'
...
// --- EVENT HANDLERS (SWAP SECTION) ---

// Connect wallet for Swap section
connectWalletSwapBtn.addEventListener('click', async () => {
    await connectWallet(walletAddressDisplaySwap, connectWalletSwapBtn, walletAddressDisplaySwap);
});

// Toggle "From" and "To" tokens
swapDirectionBtn.addEventListener('click', () => {
    const fromVal = swapFromTokenSelect.value;
    const fromText = swapFromTokenSelect.querySelector(`option[value="${fromVal}"]`).textContent;
    const fromMint = swapFromTokenSelect.querySelector(`option[value="${fromVal}"]`).dataset.mint;

    const toVal = swapToTokenSelect.value;
    const toText = swapToTokenSelect.querySelector(`option[value="${toVal}"]`).textContent;
    const toMint = swapToTokenSelect.querySelector(`option[value="${toVal}"]`).dataset.mint; // Corrected: use querySelector for data-mint

    // Temporarily store current values
    const tempFromAmount = swapFromAmountInput.value;
    const tempToAmount = swapToAmountInput.value;

    // Clear existing options before adding new ones
    swapFromTokenSelect.innerHTML = '';
    swapToTokenSelect.innerHTML = '';

    const newFromOption = document.createElement('option');
    newFromOption.value = toVal;
    newFromOption.textContent = toText;
    newFromOption.dataset.mint = toMint;
    swapFromTokenSelect.appendChild(newFromOption);

    const newToOption = document.createElement('option');
    newToOption.value = fromVal;
    newToOption.textContent = fromText;
    newToOption.dataset.mint = fromMint;
    swapToTokenSelect.appendChild(newToOption);


    // Restore values and update balances
    // If swapped, what was "To" becomes "From" for input, and vice versa for display
    swapFromAmountInput.value = tempToAmount;
    swapToAmountInput.value = tempFromAmount; // This will likely be cleared by input/change listeners, which is fine
    updateSwapBalances();
    // Reset quote
    currentQuote = null;
    executeSwapBtn.style.display = 'none';
...
// Track changes in "Amount" field or "From Token" to get a new quote
swapFromAmountInput.addEventListener('input', () => {
    currentQuote = null; // Reset quote when amount changes
    executeSwapBtn.style.display = 'none'; // Hide "Execute Swap" button
    swapToAmountInput.value = ''; // Clear "To" field
});

swapFromTokenSelect.addEventListener('change', () => {
    updateSwapBalances(); // Update balance when token changes
    currentQuote = null; // Reset quote when token changes
    executeSwapBtn.style.display = 'none'; // Hide "Execute Swap" button
    swapToAmountInput.value = ''; // Clear "To" field
});
...
maxAmountBtns.forEach(button => {
    button.addEventListener('click', async (event) => {
        ...
        if (fromTokenMint.toBase58() === TOKEN_MINT_ADDRESSES['SOL'].toBase58()) {
            ...
            // For SPL tokens: take the entire balance
            try {
                const tokenAccount = await connection.getParsedTokenAccountsByOwner(
                    currentWallet.publicKey,
                    { mint: fromTokenMint }
                ...
                if (tokenAccount.value.length > 0) {
                    const amount = tokenAccount.value[0].account.data.parsed.info.tokenAmount.amount;
                    const decimals = tokenAccount.value[0].account.data.parsed.info.tokenAmount.decimals;
                    inputElement.value = formatBigInt(amount, decimals);
                } else {
                    inputElement.value = '0';
                }
            } catch (error) {
                console.error('Error fetching max token balance:', error);
                showNotification('Error getting max balance.', 'error');
                inputElement.value = '0';
            }
        }
        // After setting MAX, reset the quote so the user gets a new one
        currentQuote = null;
        executeSwapBtn.style.display = 'none';
        swapToAmountInput.value = '';
    });
...
// Initialization - on page load, if wallet is already connected, update UI for SWAP section
document.addEventListener('DOMContentLoaded', () => {
    if (window.solana && window.solana.isPhantom && window.solana.publicKey) {
        const publicKey = window.solana.publicKey.toString();
        walletAddressDisplaySwap.textContent = `Connected: ${publicKey.substring(0, 4)}...${publicKey.substring(publicKey.length - 4)}`;
        connectWalletSwapBtn.style.display = 'none';
        walletAddressDisplaySwap.style.display = 'block';
...
// NOTE: AFOX_TOKEN_MINT_ADDRESS is already defined above, but if it needs to be different for staking vs. swap,
//       you might need to rename one or ensure consistency. Using the one defined at the top for consistency.
// const AFOX_TOKEN_MINT_ADDRESS_STAKING = new SolanaWeb3.PublicKey('GDk2UfqfWPepQBZB8BMVMCUnbLbhRCuhK6VzLBpR2HAr'); // This line was duplicated and might cause confusion.
// Using the top-level AFOX_TOKEN_MINT_ADDRESS: GLkewtq8s2Yr24o5LT5mzzEeccKuSsy8H5RCHaE9uRAd
...
// This was missing from the provided code snippet, but used in the DOMContentLoaded
