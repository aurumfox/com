...
// =========================================================================================
// 🚨 ⚠️ ⚠️ REQUIRED CHANGES BY USER (CRITICAL CHANGES) ⚠️ ⚠️ 🚨
// =========================================================================================

// 1. CRITICAL: REPLACE THIS IDL WITH THE FULL IDL GENERATED BY YOUR RUST CONTRACT!
// Find file: target/idl/alphafox_staking.json (or similar name) and copy its content here.
const STAKING_IDL = {
    version: "0.1.0",
...
// 2. STAKING ACCOUNT SEED (Key word for PDA from your Rust code)
const STAKING_ACCOUNT_SEED = "alphafox_staking_pda";

// 3. RPC Proxy (Your Cloudflare Worker)
const HELIUS_BASE_URL = 'https://solana-api-proxy.wnikolay28.workers.dev/v0/addresses/'; 

// =========================================================================================
// PROJECT CONSTANTS (WITH INTEGRATED PROGRAM ID)
// =========================================================================================

// --- CRITICAL POOL KEYS (REQUIRED TO UPDATE AFTER initialize) ---
// ⚠️ IMPORTANT: REPLACE THESE PLACEHOLDERS WITH REAL PDA AND VAULT ADDRESSES!
// You must obtain them by running the contract's 'initialize' instruction.
...
const POOLS_CONFIG = [
    ...
    { name: 'Flexible', duration_days: 0, apr_rate: 100, vote_multiplier: 1 }, // Index 4 (or special index)
]; 

const AFOX_TOKEN_MINT_ADDRESS = new window.SolanaWeb3.PublicKey(AFOX_MINT);
// 🔑 INTEGRATED PROGRAM ID (Updated to your Program ID)
...
function toggleMenuState(forceClose = false) {
    ...
    if (!menuToggle || !navOverlay || !mainNav) return;
...
async function connectWallet(adapter) {
    ...
    try {
        ...
        // setLoadingState(false) is called in simulateConnectButtonUpdate
    }
}
...
/**
 * Updates the staking UI elements with current user data.
 */
...
async function fetchUserStakingData() {
    ...
    // ⚠️ IMPORTANT WARNING: Check if AFOX_POOL_STATE_PUBKEY is not a placeholder 
    // before attempting to find the PDA. If it is a placeholder, it will fail.
    if (AFOX_POOL_STATE_PUBKEY.toBase58().includes('Placeholder')) {
         console.warn("AFOX_POOL_STATE_PUBKEY is a placeholder. Staking data fetch skipped.");
         return;
    }
    ...
    try {
        ...
        try {
            ...
            // ⚠️ IMPORTANT: Ensure the IDL contains the correct fields (stakedAmount, rewardsAmount, lockupEndTime, poolIndex, lending)
            ...
            // Account not found -> means user has not staked yet.
...
async function handleStakeAfox() {
    ...
    if (AFOX_POOL_STATE_PUBKEY.toBase58().includes('Placeholder')) {
         showNotification('Staking is not initialized. Pool addresses are placeholders. Initialize the contract first!', 'error', 10000);
         return;
    }
...
async function handleClaimRewards() {
    ...
    if (AFOX_POOL_STATE_PUBKEY.toBase58().includes('Placeholder')) {
         showNotification('Staking is not initialized. Pool addresses are placeholders.', 'error', 10000);
         return;
    }
...
/**
 * Manually disconnects the wallet.
 */
