...
function formatBigInt(amount, decimals) {
    ...
    // Ensure amount is a BN object if it's not already
...
async function getQuote() {
    ...
    const decimalsFrom = (fromMint.toBase58() === TOKEN_MINT_ADDRESSES['SOL'].toBase58()) ? SolanaWeb3.LAMPORTS_PER_SOL.toString().length - 1 : afoxDecimals;
    ...
    try {
        const response = await fetch(`${JUPITER_API_URL}/quote?inputMint=${fromMint.toBase58()}&outputMint=${toMint.toBase58()}&amount=${inputAmountLamports}&slippageBps=50`); // slippageBps=50 is 0.5% slippage
        if (!response.ok) {
        ...
        const outputAmount = new BN(currentQuote.outAmount);
        const outputDecimals = (toMint.toBase58() === TOKEN_MINT_ADDRESSES['SOL'].toBase58()) ? SolanaWeb3.LAMPORTS_PER_SOL.toString().length - 1 : afoxDecimals; // Decimal places for the target token
        ...
        lpFeeSpan.textContent = `${formatBigInt(new BN(currentQuote.lpFee.amount), outputDecimals)} ${swapToTokenSelect.value}`; // LP fee in tokens
        minReceivedSpan.textContent = `${formatBigInt(new BN(currentQuote.otherAmountThreshold), outputDecimals)} ${swapToTokenSelect.value}`; // Minimum amount to receive
...
swapDirectionBtn.addEventListener('click', () => {
    ...
    // Restore amounts and update balances
...
const STAKING_PROGRAM_ID = new SolanaWeb3.PublicKey('3GcDUxoH4yhFeM3aBkaUfjNu7xGTat8ojXLPHttz2o9f'); // THIS ADDRESS YOU STILL NEED TO FIND AND REPLACE
...
function handlePublicKeyChange(publicKey) {
    if (publicKey) {
        ...
        if (walletAddressDisplayNft) walletAddressDisplayNft.textContent = walletPublicKey.toBase55();
...
async function connectWallet() {
    try {
        ...
        await loadUserNFTs(walletPublicKey.toBase55());
...
const afoxDecimals = 6; // Standard number of decimal places for SPL tokens
...
async function getUserStakingAccount(userPublicKey) {
    console.log("Getting user staking account for:", userPublicKey.toBase58());
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (stakeAfoxBtn) {
        stakeAfoxBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const [userStakingAccountPubKey] = await SolanaWeb3.PublicKey.findProgramAddress(
                    [walletPublicKey.toBuffer(), Buffer.from("stake_account_seed")], // Real seed
                    STAKING_PROGRAM_ID
                );
                const [poolAccountPubKey] = await SolanaWeb3.PublicKey.findProgramAddress(
    ...
    if (nftDetailBuyBtn) {
        nftDetailBuyBtn.onclick = async () => {
            ...
            try {
                showNotification(`Buying ${nftToBuy.name} for ${nftToBuy.price} SOL...`, 'info', 5000);
                ...
                showNotification(`Successfully purchased ${nftToBuy.name}!`, 'success', 5000);
    ...
    copyButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (textToCopyElement && textToCopyElement.classList.contains('highlight-text')) {
                ...
                console.warn('Target element for copying not found. Make sure .copy-btn is immediately after a span with class .highlight-text.');
            }
        });
    });
