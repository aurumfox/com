...
// ⚠️ ЗАМЕНИТЕ ЭТОТ ID НА ВАШ ФАКТИЧЕСКИЙ PROGRAM ID ПОСЛЕ РАЗВЕРТЫВАНИЯ!
...
pub mod alphafox_staking {
    ...
    pub fn update_admin_fee_destination(ctx: Context<UpdateAdminFeeDestination>) -> Result<()> {
        ...
        // 🛡️ НОВАЯ ПРОВЕРКА: Предотвращение назначения Vault в качестве сбора комиссий
...
// ======================================================================
// 🧩 КОНТЕКСТЫ АККАУНТОВ (С ФИНАЛЬНЫМИ ИСПРАВЛЕНИЯМИ)
// ======================================================================
...
pub struct InitializePool<'info> {
    ...
    // 🛡️ ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ (3.2): Валидация DAO Treasury, чтобы оно принадлежало Governance Authority
...
pub struct ClaimRewards<'info> {
    ...
    // 🛡️ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Гарантия, что staker контролирует этот счет
    #[account(mut, token::mint = pool_state.staking_mint, token::authority = staker)] 
...
pub struct ReleaseDaoFunds<'info> {
    ...
    // 🛡️ ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ (1.1): Валидация конечного счета, чтобы он принадлежал Governance Authority
...
impl UserStakingAccount {
    // 8 + 32 + (8*4) + 1 + 2 + 8 + 8 + 8 + 2 + 10
    pub const LEN: usize = 8 + 32 + 8 + 8 + 8 + 8 + 1 + 2 + 8 + 8 + 8 + 2 + 10;
}
